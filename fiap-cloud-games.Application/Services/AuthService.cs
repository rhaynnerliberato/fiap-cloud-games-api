...
namespace fiap_cloud_games.Application.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<string> Authenticate(AuthCredentials credentials)
        {
            _logger.LogInformation("Autenticação iniciada para o e-mail: {Email}", credentials.Email);

            var usuario = await _usuarioRepository.ObterPorEmailAsync(credentials.Email);
            ...
            if (usuario != null)
            {
                ...
                _logger.LogWarning("Senha inválida fornecida para o e-mail: {Email}", credentials.Email);
            }
            else
            {
                _logger.LogWarning("Usuário não encontrado para o e-mail: {Email}", credentials.Email);
            }
